...
> 我想让客户端连接服务器，但连接失败。  
> 我不确定应该开放哪个端口。  
> 当前配置里 `server-port=25565`，但我看到还有 `query.port`、`rcon.port`、`management-server-port`。  
> 请问客户端联机需要开放哪个端口？
...
- **“我不确定应该开放哪个端口。”**
- **“我不确定 `server-port` 和 `query.port` 哪个才是客户端连接端口。”**
- **“请问下一步应该检查防火墙、端口映射还是配置文件？”**

推荐写法示例：

> 我想让别人连接我的 Minecraft 服务器。  
> 目前服务器配置里有这些端口：  
> `server-port=25565`  
> `query.port=25565`  
> `rcon.port=25575`  
