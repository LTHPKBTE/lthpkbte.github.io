...
### L.005 提问时至少要真的提出一个问题

错误示例：

- 只发一张截图，然后说：**“有人帮我看下吗？”**
- 或者重复：**“一样结果。”**
- 只描述零散现象，但没有说明你到底想让别人判断什么

为什么：

因为这些内容严格来说并不是“问题”，而只是：
...
回答者看到这类内容时，通常无法判断你到底需要什么帮助。例如回答者可能不知道你是在问：
...
这会导致回答者必须先反过来帮你整理问题，甚至替你提问。  
例如原本你可以直接说明：

> 我想在服务器上部署一个 Web 服务，但外部访问不到。
> 我不确定是 Nginx 配置问题还是防火墙问题。
> 当前 Nginx 监听 80 端口，`ufw status` 显示 80 已放行，但 `curl localhost` 正常、外网访问却返回 Connection Refused。
> 请问下一步应该检查哪里？
...
这类提问的问题不在于“信息不够完整”，而在于**连基本的问题目标都没有说出来**。
...
- **"外网访问返回 Connection Refused / 超时 / 端口检测不通。"**
- **"程序启动时报连接超时，但目标服务是运行中的。"**

你希望别人帮你判断什么

例：

- **"我不确定问题是出在应用配置、系统防火墙还是云服务商的安全组。"**
- **"我不确定连接字符串里的 host 应该填内网地址还是公网地址。"**
- **"请问下一步应该检查哪一层：应用配置、系统防火墙还是网络策略？"**

推荐写法示例：

> 我想在一台刚装完系统的 VPS 上部署一个 API 服务，让它能通过公网被调用。
> 目前的情况：
> - 服务已启动，监听 `127.0.0.1:3000`
> - `curl http://127.0.0.1:3000/api/health` 在本机正常返回
> - 但用公网 IP 访问时直接超时
>
> 我的问题是：
> **从本机 curl 通但外网不通，应该从哪一层开始排查？**
>
> 我已经做过：
> - 确认服务正在运行
> - 检查云服务商控制台的安全组，已放行 3000 端口
> - 用 `ss -tlnp` 确认端口在监听
>
> 但现在外网仍然无法访问，想确认下一步应该排查哪里。

如果你只是想让别人帮你看截图，也应当说明"看什么"。
...
- 截图不能替代问题本身。
- 情绪可以表达，但不能代替问题描述。
- 如果你自己都说不清想问什么，别人通常也无法有效回答。
- 提问前先把问题写成一句完整的话：  
  **“我想做 X，但现在出现 Y，我想确认 Z。”**
